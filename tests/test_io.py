...
def test_rw():
    ...
    # The input effectivley becomes 0x10x2, thus 0x10
    # The weird concatination + parse result is a number parsing problem in bmath.
    with open("/dev/bmath", "r+") as f:
        f.write("0x1")
        f.write("0x2")
        data = f.read()
        assert "Hex: 0x10" in data, "second write concats to first"
